...
start_containers() {
    message "Restarting the split service" 0 6
    sudo -u"$LOCAL_USER" VAULT_URL="$VAULT_URL" VAULT_ROLE_ID="$VAULT_ROLE_ID" VAULT_SECRET_ID="$VAULT_SECRET_ID" docker compose --profile pdf_split_service --env-file .env.local down
